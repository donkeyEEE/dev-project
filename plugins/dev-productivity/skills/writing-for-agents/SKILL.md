...
description: Use when creating, reviewing, or editing skills, AGENTS.md, agents/openai.yaml, plugin manifests, marketplace metadata, or other operational documents that agents consume.
---

Reference for writing any document an agent consumes: a skill, an `AGENTS.md`, plugin metadata, or a doc reached by a pointer. The packaging differs; the writing does not: the same levers make each one predictable: the agent taking the same _process_ every run, not producing the same output.
